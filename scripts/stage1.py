...
def query(driver, start_date, end_date):
    ...
    # Click "Add a rule"
    filter_dropdown_trigger = driver.find_element_or_wait(
        By.CSS_SELECTOR, '.filter-dropdown-trigger')
    ...
    input_date_from = driver.find_element_or_wait(
        By.CSS_SELECTOR, 'input[id$="filter-field-date-from"]')
    input_date_to = driver.find_element_or_wait(
        By.CSS_SELECTOR, 'input[id$="filter-field-date-to"]')
...
def get_n_pages(driver):
    try:
        last_a = driver.find_element_or_wait(
            By.CSS_SELECTOR, 'a[title="Go to last page"]', timeout=1)
