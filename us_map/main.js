...
let map_width = MAX_WIDTH, map_height = MAX_HEIGHT;
...
projection.translate([map_width/2 - margin.left, map_height/2 - margin.top]).scale(1400).center([-95.5,40]);
...
function updateMap(){
    d3.csv(filename).then(function(data){
        ...
        countymap.then(function(values){
            ...
            let counties = mapsvg.selectAll("path")
            ...
            // draw map
                ...
                .attr("d", path)
                .style("fill", (d)=>countyColor(d.properties.NAME, d.properties.STATE, data, maxVal));
...
function countyColor(county, state, data, maxVal){
    ...
    for (let i = 0; i < data.length; i++){
        if (data[i]["County"] === county){
            color = [goodColor[0], goodColor[1], goodColor[2], data[i][curr_field]/maxVal];
